...
function utility:Create(MID, Callback, CD, MName)
  if tools[1] and type(tools[1])=="string" then
    notif("Starting:", Moveset .." Custom Moveset, uses Aedaniss7's custom movesets V2 utility")
    for i, moveName in ipairs(tools)
...
notif("Encountered Fatal Error:", "<whatever value this is put in as in ur script>:ReqChr('character') has not been ran! this is vital for setting up moves!")
end
  local moveSlot, err=pcall(function()
      local MS=hotbar[tostring(MID)]
      return MS
    end)
  ...
  if isCustom then
    moveSlot.Base.MouseButton1Click:Connect(function() if customSlotCD(CD, MID) then
      Callback()
    end end)
...
function utility:SFX(music, url, path)
  if not isfile(path) then
    writefile(parh, game:HttpGet(url))
  print(path .." Saved!"
  else
